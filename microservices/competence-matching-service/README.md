...
curl -X POST http://localhost:8094/v1/subscriptions \
  ...
  -d '{"skill":"java","rating":2,"email":"anna.korhonen@example.com"}'
```
...
```bash
docker buildx build --platform linux/amd64,linux/arm64 -t miguelsombrero/subscription-matching-service:latest --push .
```
